...
the one guard on duty that night - the Elves seem to have decided that
...
def main():
    ...
    guard = None
    since = 0
    guard_asleep_total = 0
    guard_asleep_minute = 0
    guard_id = None
    guard_id2 = None
    guard_asleep_shift_sum = 0
    guard_asleep_shift_minute = 0
    ...
    for i, line in enumerate(data):
        if ' 23:' in line:
            day = data[i][9:11]
            what = data[i][9:17]
            data[i] = data[i].replace(what, "%02d 00:00" % (int(day)+1))

    # Order and parse messages
    for line in sorted(data):
        date, msg = line.split('] ')
        month, day, hour, minute = map(int, [
            date[6:8], date[9:11], date[12:14], date[15:]])
        ...
        if msg[0] == 'G':
            ...
            records[guard].append([False] * 60)
            since = 0
        else:
            if msg[0] == 'f':
                since = minute
            else:
                records[guard][-1][since:minute] = [True] * (minute - since)
    ...
    for guard in records:
        if guard is not None:
            ...
            for time in range(60):
                ...
                for shift in records[guard]:
                    if shift[time]:
                        shifts_sum += 1
                        total_asleep += 1

                if shifts_sum > shifts_sum_max:
                    shifts_sum_max = shifts_sum
                    shifts_sum_minute = time

            if total_asleep > guard_asleep_total:
                guard_id = guard
                guard_asleep_total = total_asleep
                guard_asleep_minute = shifts_sum_minute

            if shifts_sum_max > guard_asleep_shift_sum:
                guard_id2 = guard
                guard_asleep_shift_sum = shifts_sum_max
                guard_asleep_shift_minute = shifts_sum_minute

    print("[Star 1] Guard ID multiplied by num: %d" % (
        guard_id * guard_asleep_minute))
    print("[Star 2] Guard ID multiplied by num: %d" % (
        guard_id2 * guard_asleep_shift_minute))
